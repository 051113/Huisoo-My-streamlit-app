...
def pick_movies(candidates, user_state, openai_api_key, feedback):
    ...
    try:
        ...
        if not recommender.is_tiered_order(picks):
            raise ValueError("Tier ordering invalid")
...
def _call_openai(candidates, user_state, openai_api_key):
    ...
    system_message = (
        ...
        "candidate list in this strict order: [Popular pick, Critically acclaimed pick, Wild card pick]. "
        "Never invent titles or IDs. Keep wildcard meaningfully different from the first two. "
